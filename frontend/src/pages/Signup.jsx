...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      console.log("Signup - Sending to Backend:", payload);
      ...
      if (err.response && err.response.status === 422) {
        ...
        alert("Input Error: " + JSON.stringify(err.response.data.detail));
      } else if (err.response && err.response.status === 400) {
        alert("Error: " + err.response.data.detail);
      } else if (!err.response) {
        alert("Connectivity Error: Backend server se baat nahi ho paa rahi. Check karein ki backend chalu hai.");
      } else {
        alert("Error: " + (err.response?.data?.detail || err.message || "Kuch galat hua"));
      }
    } finally {
      setLoading(false);
    }
